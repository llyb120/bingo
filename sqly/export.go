...
//
// )
var dbUtil = core.Use[DBUtil]()
var DefaultDBUtil = &defaultDBUtil{}

func getDbUtil() DBUtil {
	util := dbUtil()
	if util == nil {
...
func Select[T any](ctx core.Context, db DBLike, sql string, args ...any) (T, error) {
	var zero T
	err := getDbUtil().Select(ctx, db, &zero, sql, args...)
	if err != nil {
		return zero, err
	}
	return zero, nil
...
func Exec(ctx core.Context, db DBLike, sql string, args ...any) (int64, int64, error) {
	return getDbUtil().Exec(ctx, db, sql, args...)
}
